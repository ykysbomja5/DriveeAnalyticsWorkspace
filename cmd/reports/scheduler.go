...
// startTemplateScheduler координирует побочные эффекты выполнения и фиксирует результат.
func (app application) startTemplateScheduler() {
	// При старте сразу проверяем ближайшие слоты, чтобы не пропустить запуск до первого тика.
...
// runDueTemplates координирует побочные эффекты выполнения и фиксирует результат.
func (app application) runDueTemplates(ctx context.Context) {
	...
	for _, template := range templates {
		...
		// Старые слоты намеренно пропускаются, чтобы сервис не догонял пачкой давно пропущенные задачи.
		if now.Sub(slot) > 2*time.Minute {
			continue
		}
		// Один и тот же слот нельзя выполнить повторно, даже если scheduler успел сработать несколько раз.
		if template.lastScheduledFor != nil && !template.lastScheduledFor.Before(slot) {
			continue
		}

		runResp, savedReport, err := app.executeTemplate(ctx, template, "scheduled", true, template.OwnerName, template.OwnerDepartment, template.IsPublic)
		if err != nil {
...
// fetchTemplateRecords загружает записи, доступные текущему пользователю или сценарию.
...
// executeTemplate координирует побочные эффекты выполнения и фиксирует результат.
func (app application) executeTemplate(ctx context.Context, template templateRecord, source string, scheduled bool, ownerName, ownerDepartment string, isPublic bool) (shared.RunResponse, shared.SavedReport, error) {
	...
	savedReport, err := app.saveRunSnapshot(ctx, reportName, template.QueryText, runResp, source, &templateID, ownerName, ownerDepartment, isPublic, 0)
	if err != nil {
		return shared.RunResponse{}, shared.SavedReport{}, err
	}
...
// markTemplateRun координирует побочные эффекты выполнения и фиксирует результат.
...
// markTemplateManualRunStarted координирует побочные эффекты выполнения и фиксирует результат.
func (app application) markTemplateManualRunStarted(ctx context.Context, templateID int64) {
	_, _ = app.db.Exec(ctx, `
		update app.report_templates
		set
			last_status = 'running',
...
// markTemplateManualRunSucceeded координирует побочные эффекты выполнения и фиксирует результат.
func (app application) markTemplateManualRunSucceeded(ctx context.Context, templateID int64, rowCount int) {
	_, _ = app.db.Exec(ctx, `
		update app.report_templates
		set
			last_run_at = now(),
...
// markTemplateManualRunFailed координирует побочные эффекты выполнения и фиксирует результат.
func (app application) markTemplateManualRunFailed(ctx context.Context, templateID int64, errorText string) {
	_, _ = app.db.Exec(ctx, `
		update app.report_templates
		set
			last_status = 'failed',
			...
			updated_at = now()
		where id = $1
	`, templateID, nullableText(errorText))
}

// humanScheduleLabel выполняет отдельный шаг окружающего сервисного сценария.
...
// dueRunForSchedule выполняет отдельный шаг окружающего сервисного сценария.
func dueRunForSchedule(schedule shared.ReportTemplateSchedule, now time.Time, fallback *time.Location) time.Time {
	...
	// Ищем последний наступивший слот недели в таймзоне самого шаблона.
...
// nextRunForSchedule выполняет отдельный шаг окружающего сервисного сценария.
...
// resolveScheduleLocation нормализует граничные значения перед дальнейшим использованием.
